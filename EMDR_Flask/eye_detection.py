...
class EyeDetection:
    ...
    def positionEstimator(self, cropped_eye):
        ...
        ret, threshed_eye = cv.threshold(median_blur, 20, 255, cv.THRESH_BINARY)
    ...
    def pixelCounter(self, first_piece, second_piece, third_piece):
        ...
        if max_index==0:
            ...
            pos_eye="Closed"
            color = [utils.GRAY, utils.YELLOW]
        #print("in pixelCounter: ",pos_eye, color)
    ...
    def getFrame(self):
        ...
        print("video")         
        print("1st step, out:", out)
        with map_face_mesh.FaceMesh(min_detection_confidence =0.5, min_tracking_confidence=0.5) as face_mesh:
            # starting time here 
            start_time = time.time()
            # starting Video loop here.
            logging.info("starting Video loop here...")
            
            frame_counter +=1 # frame counter
            print("frame_counter", frame_counter) 
                
            ret, frame = video.read() # getting frame from camera 
                # Video resoulation 
                #VIDEO_WIDTH = 1080
                #VIDEO_HEIGHT = 1440
            
            frame_witdh, frame_height = frame.shape[:2]
            if out is None:
                logging.info("Creating output file...")
                now = datetime.now()
                now_str = now.strftime('%Y%m%d_%H%M%S')
                #fourcc = cv.VideoWriter_fourcc(*'XVID')
                #out = cv.VideoWriter(f'output_{now_str}.avi', fourcc, 20, (frame_height, frame_witdh))

            # frame = cv.resize(frame, None, fx=1.5, fy=1.5, interpolation=cv.INTER_CUBIC)
            rgb_frame = cv.cvtColor(frame, cv.COLOR_RGB2BGR)
            print("rgb_frame", rgb_frame)
            print("frame_size", frame_height, frame_witdh)
        ...
        # Blink Detector Counter Completed
            ...
            ratio = 30
            crop_right = cv.resize(crop_right, (crop_right.shape[1] * ratio, crop_right.shape[0] * ratio))
            crop_left = cv.resize(crop_left, (crop_left.shape[1] * ratio, crop_left.shape[0] * ratio))
            ...
            #print("in main right: ",eye_position, color)
            #eye_position_left, color = positionEstimator(crop_left)
            utils.colorBackgroundText(frame, f'L: {eye_position}', FONTS, 1.0, (40, 320), 2, color[0], color[1], 8, 8)
            #print("in main left: ",eye_position, color)
        ...
        if out is not None:
            logging.info("Releasing output file...")
            out.release()
